...
- `SENSOR_HOST=sensor-tcp-simulator`
- `SENSOR_PORT=7000`

Notes:

- In `compose.yml`, `sensor-tcp-simulator` is the Docker Compose service name of the Sensor TCP Simulator.
- The IoT Gateway should connect to `sensor-tcp-simulator:7000` from inside the Compose network.
...
- Sender may produce random bursts of data.
...
The IoT Gateway should log parse errors and continue.
